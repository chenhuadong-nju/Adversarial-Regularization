...
class ChoiPyramid(nn.Module):
    ...
    def run_embed(self, x):
        ...
        embeds = torch.cat([b.unsqueeze(0)
                            for b in torch.chunk(embeds, batch_size, 0)], 0) # batch_size, 1, seq_length, model_dim
        embeds = F.dropout(
            embeds,
    ...
    def forward(
        ...
        x, example_lengths = self.unwrap(sentences, example_lengths)  # x: 2nsamples * length  example_lengths:  2nsamples * 1
        ...
        hh, cc, masks, temperature = self.binary_tree_lstm(
    ...
    def unwrap_sentence_pair(self, sentences, lengths=None):
        x_prem = sentences[:, :, 0]    # nsamples * length
        x_hyp = sentences[:, :, 1]
        x = np.concatenate([x_prem, x_hyp], axis=0)  # 2nsamples * length
...
class BinaryTreeLSTM(nn.Module):
    ...
    def forward(self, input, length, temperature_multiplier=1.0):
        ...
        for i in range(max_depth - 1):
            h, c = state
            l = (h[:, :-1, :], c[:, :-1, :])   # 0 : (seq_len - 2)
            r = (h[:, 1:, :], c[:, 1:, :])     # 1 : (seq_len - 1)
...
def sequence_mask(sequence_length, max_length=None):
    if max_length is None:
        max_length = sequence_length.data.max()   # Variable.data
    ...
    # batch_size * max_length;  batch_size * 1
    # set elements of each row in seq_range_expand to 1 if it less than element in corresponding row of seq_length_expand
