...
def Peano(x, y):
    # transfer to binary code, then join by bit 001 010 as 000110 finally recover to a number
...
def MakeTrainingIterator(
    ...
    def build_batches():
        dataset_size = len(sources[0])   # X
        order = range(dataset_size)
        random.shuffle(order)
        order = np.array(order)

        num_splits = 10  # TODO: Should we be smarter about split size?
        order_limit = len(order) / num_splits * num_splits  # if len is 115 output 110
        order = order[:order_limit]
        order_splits = np.split(order, num_splits)   # split order into 10 pieces
        batches = []
...
def PreprocessDataset(
    ...
    if sentence_pair_data:
        X = np.transpose(np.array([[example["premise_tokens"] for example in dataset],
                                  ...
                                  dtype=np.int32), (1, 2, 0))  # n_samples * length * 2

        left, right, write = ActionsToArray(dataset)
        ...
        if simple:
            ...
            num_transitions = np.transpose(np.array(
                ...
                dtype=np.int32), (1, 0))           # n_samples * 2, each element means the length of each sample
        else:
    ...
    return X, transitions, y, num_transitions, example_ids, left, right, write

def ActionsToArray(dataset):
    keys = ["sentence1_acts",
            "sentence2_acts"]
    lefts1 = []
    rights1 = []
    ...
    for example in dataset:
        ...
        #print(example[keys[1]])
        for action in example[keys[0]]:
            action_seq = action.split(",")
            #print(action_seq)
            left1.append(int(action_seq[0]) - 1)
            right1.append(int(action_seq[1]) - 1)
            write1.append(int(action_seq[2]))
        for action in example[keys[1]]:
            action_seq = action.split(",")
            #print(action_seq)
            left2.append(int(action_seq[0]) - 1)
            right2.append(int(action_seq[1]) - 1)
            write2.append(int(action_seq[2]))
        lefts1.append(left1)
        rights1.append(right1)
        writes1.append(write1)
        lefts2.append(left2)
        rights2.append(right2)
        writes2.append(write2)
    #temp = np.array([lefts1, lefts2], dtype=np.int32)
    #print(temp.shape)
    left = np.transpose(np.array([lefts1, lefts2], dtype=np.int32), (1, 2, 0))
    right = np.transpose(np.array([rights1, rights2], dtype=np.int32), (1, 2, 0))
    write = np.transpose(np.array([writes1, writes2], dtype=np.int32), (1, 2, 0))
    return left, right, write
