...
class GRU(nn.Module):
    def __init__(self, inp_dim, model_dim, num_layers=1,
        ...
        self.rnn = nn.GRU(inp_dim, model_dim / self.bi, num_layers=num_layers,    # number of features of input and hidden, number of layers
                          batch_first=True,    # the input and output tensors are provided as (batch, seq, feature)
                          bidirectional=self.bidirectional)    # use bidirectional GRU
...
class EncodeGRU(GRU):
    ...
    def forward(self, x, h0=None):
        ...
        if self.mix:   # means seq gru(h) with tree lstm(h,c)
