...
def get_batch(batch):
    X_batch, transitions_batch, y_batch, num_transitions_batch, example_ids, left, right, write = batch
    ...
    left = truncate(left, seq_length, max_length, X_batch_is_left_padded)
    right = truncate(right, seq_length, max_length, X_batch_is_left_padded)
    write = truncate(write, seq_length, max_length, X_batch_is_left_padded)

    return X_batch, transitions_batch, y_batch, num_transitions_batch, example_ids, left, right, write
...
def load_data_and_embeddings(
    ...
    if not data_manager.FIXED_VOCABULARY:
        ...
        # spinn.util.data.BuildVocabulary
...
def get_flags():
    ...
    gflags.DEFINE_float("learning_rate_d", 0.001, "Used in discriminator optimizer.")
    gflags.DEFINE_float(
...
def init_model(
    ...
    if FLAGS.encode == "projection":
        ...
        context_args.reshape_input = lambda x, batch_size, seq_length: x.view(
            batch_size, seq_length, -1)    # view map x to shape [batch_size, seq_length, rest]
    ...
    gold_tree = build_gtree(data_manager, initial_embeddings, vocab_size, FLAGS, context_args)
    ...
    if FLAGS.optimizer_type == "Adam":
        ...
        optimizer_D = optim.Adam(discriminator.parameters(), lr=FLAGS.learning_rate_d,
                                 betas=(0.9, 0.999), eps=1e-08)
        optimizer_tree = optim.Adam(gold_tree.parameters(), lr=FLAGS.learning_rate_d,
                                 betas=(0.9, 0.999), eps=1e-08)
